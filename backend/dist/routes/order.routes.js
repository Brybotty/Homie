...
router.patch('/:id/shipping', authMiddleware_1.requireAdmin, (0, validateDto_1.validate)([
    (0, express_validator_1.param)('id').isInt().withMessage('El ID debe ser un número entero'),
    (0, express_validator_1.body)('shipping_cost').isFloat({ min: 0 }).withMessage('El costo de envío debe ser un número positivo'),
]), controller.updateShipping);
// DELETE /api/orders/:id — Eliminar un pedido (solo administrador)
router.delete('/:id', authMiddleware_1.requireAdmin, (0, validateDto_1.validate)([(0, express_validator_1.param)('id').isInt().withMessage('El ID debe ser un número entero')]), controller.deleteOrder);
