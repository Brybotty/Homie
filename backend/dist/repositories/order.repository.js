...
class OrderRepository {
    ...
    async createOrder(dto) {
        ...
        try {
            ...
            const custFindRes = await client.query(custFindQuery, [dto.customer.phone]);
            let customer;
          ...
          RETURNING *
        `;
                const updateCustRes = await client.query(updateCustQuery, [
          ...
          RETURNING *
        `;
                const insertCustRes = await client.query(insertCustQuery, [
                    dto.customer.full_name,
                    dto.customer.email || null,
                    dto.customer.phone,
                    dto.customer.document_id || null,
        ...
        `;
                ...
                if (variantRes.rows.length === 0) {
                    throw new errorHandler_1.AppError(`Variante con ID ${item.variant_id} no existe`, 400);
                }
                const variant = variantRes.rows[0];
                if (!variant.is_active) {
                    throw new errorHandler_1.AppError(`La variante '${variant.variant_name}' no está activa`, 400);
                }
                if (variant.stock_quantity < item.quantity) {
                    throw new errorHandler_1.AppError(`Stock insuficiente para '${variant.product_name} - ${variant.variant_name}'. Disponible: ${variant.stock_quantity}, Solicitado: ${item.quantity}`, 400);
                }
    ...
    async findAll(options) {
        ...
        const countRes = await database_1.pool.query(countQuery, values);
    ...
    `;
        ...
        const result = await database_1.pool.query(dataQuery, values);
        ...
        try {
            ...
            if (targetStatus && targetStatus !== previousStatus) {
                ...
                if (targetStatus === 'CANCELADO' && previousStatus !== 'CANCELADO') {
                 ...
                 WHERE id = $2`, [item.quantity, item.variant_id]);
                        }
                    }
                }
                // Si se reactiva una orden que estaba CANCELADA -> VOLVER A DESCONTAR STOCK
                    ...
                    for (const item of items) {
                        if (item.variant_id) {
                            const variantRes = await client.query('SELECT stock_quantity, variant_name FROM product_variants WHERE id = $1 FOR UPDATE', [item.variant_id]);
                            if (variantRes.rows.length > 0) {
                                const available = variantRes.rows[0].stock_quantity;
                                if (available < item.quantity) {
                                    throw new errorHandler_1.AppError(`Stock insuficiente para reactivar el pedido. Disponible: ${available}, Requerido: ${item.quantity}`, 400);
                                }
                            }
                            await client.query(`UPDATE product_variants 
                 SET stock_quantity = stock_quantity - $1, updated_at = CURRENT_TIMESTAMP 
    ...
    async getFinancialSummary() {
        ...
        return result.rows.map((row) => ({
            ...row,
            revenue_total: parseFloat(row.revenue_total),
            total_cogs_mayorista: parseFloat(row.total_cogs_mayorista),
            shipping_cost: parseFloat(row.shipping_cost),
