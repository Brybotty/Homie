...
class ProductRepository {
      ...
      `;
            ...
            if (err.code === '23505') {
                if (err.constraint?.includes('sku')) {
                    throw new errorHandler_1.AppError('El SKU de una de las variantes ya está en uso. Cada variante debe tener un SKU único.', 409, err.detail);
                }
                if (err.constraint?.includes('slug')) {
                    throw new errorHandler_1.AppError('El slug ya está en uso por otro producto.', 409, err.detail);
                }
            }
            throw err;
        }
    ...
    async updateWithVariants(id, dto) {
        const client = await database_1.pool.connect();
        try {
            await client.query('BEGIN');
            const fields = [];
        ...
        `;
            ...
            if (dto.variants && Array.isArray(dto.variants)) {
                ...
                const currentVariantsRes = await client.query('SELECT id FROM product_variants WHERE product_id = $1', [id]);
             ...
             WHERE product_id = $1 
               AND id = ANY($2::int[])
               AND id NOT IN (SELECT variant_id FROM supplier_batch_items WHERE variant_id IS NOT NULL)`, [id, idsToRemove]);
                    await client.query(`UPDATE product_variants 
             SET is_active = false 
             WHERE product_id = $1 
               AND id = ANY($2::int[])`, [id, idsToRemove]);
                }
                for (const v of dto.variants) {
                    if (v.id) {
    ...
    async updateBase(id, dto) {
        ...
        if (fields.length === 0) {
            const res = await database_1.pool.query('SELECT * FROM products WHERE id = $1', [id]);
    ...
    async updateVariantStock(variantId, stockQuantity) {
        const query = `
      UPDATE product_variants
      ...
      RETURNING *
    `;
        const result = await database_1.pool.query(query, [stockQuantity, variantId]);
        return result.rows[0] || null;
    }
    async updateVariant(variantId, dto) {
        ...
        if (fields.length === 0) {
            const res = await database_1.pool.query('SELECT * FROM product_variants WHERE id = $1', [variantId]);
            return res.rows[0] || null;
        }
