...
export class OrderRepository {
  ...
  async updateShipping(id: number, shippingCost: number): Promise<Order | null> {
  ...
  async deleteOrder(id: number): Promise<void> {
    ...
    try {
      ...
      const orderRes = await client.query<Order>('SELECT * FROM orders WHERE id = $1 FOR UPDATE', [id]);
      ...
      if (order.order_status !== 'CANCELADO') {
        const itemsRes = await client.query<OrderItem>('SELECT * FROM order_items WHERE order_id = $1', [id]);
