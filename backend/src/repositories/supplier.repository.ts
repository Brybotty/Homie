...
export class SupplierRepository {
  async getCurrentBatch(): Promise<(SupplierBatch & { items: SupplierBatchItem[] }) | null> {
    const batchQuery = `
      ...
      LIMIT 1
    `;
    const batchRes = await pool.query<SupplierBatch>(batchQuery);
    ...
    `;
    const itemsRes = await pool.query(itemsQuery, [batch.id]);

    return {
      ...batch,
      total_cost: parseFloat(batch.total_cost as any),
  ...
  async createOrConsolidateBatch(options: {
    ...
    try {
      ...
      const currentRes = await client.query<SupplierBatch>(
        `SELECT * FROM supplier_batches WHERE status = 'EN_ACUMULACION' ORDER BY created_at DESC LIMIT 1`
      );
      ...
      if (currentRes.rows.length > 0) {
        ...
        const insertBatchQuery = `
          ...
          RETURNING *
        `;
        const newBatchRes = await client.query<SupplierBatch>(insertBatchQuery, [
          batchCode,
      ...
      if (options.items && options.items.length > 0) {
        for (const item of options.items) {
          ...
          const existingItemRes = await client.query<SupplierBatchItem>(
            `SELECT * FROM supplier_batch_items WHERE batch_id = $1 AND variant_id = $2`,
            [batch.id, item.variant_id]
          );

          if (existingItemRes.rows.length > 0) {
            await client.query(
              `UPDATE supplier_batch_items 
               SET quantity_ordered = quantity_ordered + $1, unit_cost_agreed = $2 
               WHERE id = $3`,
              [item.quantity, unitCost, existingItemRes.rows[0].id]
            );
          } else {
            await client.query(
              `INSERT INTO supplier_batch_items (batch_id, variant_id, quantity_ordered, unit_cost_agreed, quantity_received)
               VALUES ($1, $2, $3, $4, 0)`,
              [batch.id, item.variant_id, item.quantity, unitCost]
            );
          }
        }
      }
      ...
      const updateBatchRes = await client.query<SupplierBatch>(
        `UPDATE supplier_batches SET total_units = $1, total_cost = $2 WHERE id = $3 RETURNING *`,
        [totalUnits, totalCost, batch.id]
      );
